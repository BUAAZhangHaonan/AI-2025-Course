# PSPNet 深度注意力配置文件 - Electronic Component 数据集
# 使用 ResNet50 + 深度引导注意力，输入尺寸512x512
...
custom_imports = dict(
    imports=[
        ...
        'rgbd_projects.electronic_component.mmseg.models.backbones.resnet_depth_attention'  # 注意力backbone
    ],
...
model = dict(
    type='EncoderDecoder',
    pretrained=None,  # 在backbone中指定预训练权重
    data_preprocessor=dict(
        type='SegDataPreProcessor',
        mean=[123.675, 116.28, 103.53, 127.5],  # RGB + Depth的均值
    ...
    backbone=dict(
        type='ResNetV1c_DepthAttention',  # 深度注意力backbone
        depth=50,
...
# 从RGB基线模型加载权重（可选，仅加载兼容的decode_head等）
