...
@DATASETS.register_module()
class ElectronicComponentRGBDDataset(BaseSegDataset):
    """Electronic Component RGBD dataset for segmentation with depth fusion.

    This dataset extends the base Electronic Component dataset to support
    RGB-D fusion by loading both RGB images and corresponding depth maps.
    ...
    Args:
        depth_suffix (str): Suffix of depth maps. Default: '.npy'
        data_prefix (dict): Should contain img_path, seg_map_path, and depth_path
    """
    ...
    def __init__(self,
                 ...
                 metainfo: Optional[dict] = None,
                 data_root: Optional[str] = None,
                 data_prefix: dict = dict(img_path='', seg_map_path='', depth_path=''),
                 filter_cfg: Optional[dict] = None,
                 indices: Optional[List[int]] = None,
                 serialize_data: bool = True,
                 pipeline: List = [],
    ...
    def load_data_list(self) -> List[dict]:
        """加载RGBD数据列表，匹配RGB图像、深度图和标签文件"""

        # 获取目录路径
        if self.data_prefix['img_path'].startswith(self.data_root):
            img_dir = self.data_prefix['img_path']
            ann_dir = self.data_prefix['seg_map_path']
            depth_dir = self.data_prefix.get('depth_path', '')
        else:
            img_dir = osp.join(self.data_root, self.data_prefix['img_path'])
            ann_dir = osp.join(self.data_root, self.data_prefix['seg_map_path'])
            depth_dir = osp.join(self.data_root, self.data_prefix.get('depth_path', ''))

        print(f"[RGBD] Loading data from:")
        print(f"  - Images: {img_dir}")
        ...
        data_list = []
        skipped = {'no_depth': 0, 'no_mask': 0}

        for img_file in sorted(img_files):
            img_name = osp.splitext(img_file)[0]
            ann_file = img_name + self.seg_map_suffix
            depth_file = img_name + self.depth_suffix
        ...
        print(f'[RGBD] Loaded {len(data_list)} samples')
        if skipped['no_mask'] > 0:
            print(f'[RGBD] Skipped {skipped["no_mask"]} (no mask)')
        if skipped['no_depth'] > 0:
            print(f'[RGBD] Skipped {skipped["no_depth"]} (no depth)')
