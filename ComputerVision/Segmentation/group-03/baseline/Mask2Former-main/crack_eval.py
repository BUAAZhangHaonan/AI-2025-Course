...
labels = [
    ...
    Label(  'road'                 ,  0   ,        0 , 'road'            , 1       , False        , False        , (128, 64,128) ),
    Label(  'crack'                ,  1   ,        1 , 'crack'           , 1       , False        , False        , (244, 35,232) ),
]
...
def getPrediction_xxxxx( args, groundTruthFile ):
...
args.avgClassSize       = {
    "road"    :  4672.3249222261 ,
    "crack"    : 36771.8241758242 ,
}
...
def createResultDict_simple( confMatrix, classScores, perImageStats, args ):
    # write JSON result file
    wholeData = {}
    wholeData["confMatrix"] = confMatrix.tolist()
    wholeData["priors"] = {}
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    if len(predictionImgList) != len(groundTruthImgList):printError("List of images for prediction and groundtruth are not of equal size.")
    confMatrix    = generateMatrix(args)
    instStats     = generateInstanceStats(args)   # args.evalInstLevelScore True时才用到
    perImageStats = {}
    nbPixels      = 0
    ...
    # sanity check
    if confMatrix.sum() != nbPixels: printError('Number of analyzed pixels and entries in confusion matrix disagree: contMatrix {}, pixels {}'.format(confMatrix.sum(),nbPixels))

    # print confusion matrix
    if (not args.quiet): printConfMatrix(confMatrix, args)
    ...
    allResultsDict = createResultDict_simple( confMatrix, classScoreList, perImageStats, args )
    # write result file
    if args.JSONOutput:
        writeJSONFile( allResultsDict, args)
    assert allResultsDict is not None
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args):
    ...
    try:
        ...
        groundTruthNp = (groundTruthNp == 255).astype(np.uint8)
    except:
    ...
    if (predictionImg.size[0] != groundTruthImg.size[0]):printError("Image widths of " + predictionImgFileName + " and " + groundTruthImgFileName + " are not equal.")
    if (predictionImg.size[1] != groundTruthImg.size[1]):printError("Image heights of " + predictionImgFileName + " and " + groundTruthImgFileName + " are not equal.")
    if ( len(predictionNp.shape) != 2 ):printError("Predicted image has multiple channels.")
