...
可通过访问[DatasetCatalog](https://detectron2.readthedocs.io/modules/data.html#detectron2.data.DatasetCatalog)来使用数据集用于其数据，或[MetadataCatalog](https://detectron2.readthedocs.io/modules/data.html#detectron2.data.MetadataCatalog)用于其元数据（类别名称等）。本文档说明了如何配置内置数据集，以便上述API能够使用这些数据集。[[Use Custom Datasets](https://detectron2.readthedocs.io/tutorials/datasets.html)提供了关于如何使用`DatasetCatalog`和`MetadataCatalog`的更深入解析，以及如何向它们添加新数据集。
...
MaskFormer 内置支持多个数据集。假设数据集存在于由环境变量指定的目录中```DETECTRON2_DATASETS```。在此目录下，detectron2 将按以下结构搜索所需的数据集。
...
您可以通过`export DETECTRON2_DATASETS=/path/to/datasets`设置内置数据集的位置。如果未设置，默认值为`./datasets`相对于当前工作目录。

The [model zoo](https://github.com/facebookresearch/MaskFormer/blob/master/MODEL_ZOO.md)
contains configs and models that use these builtin datasets.

[model zoo](https://github.com/facebookresearch/MaskFormer/blob/master/MODEL_ZOO.md)
包含使用这些内置数据集的配置和模型。
...
然后，运行`python datasets/prepare_coco_semantic_annos_from_panoptic_annos.py`，从全景注释中提取语义注释（仅用于评估）
...
注意：要创建labelTrainIds.png，首先准备上述结构，然后使用以下命令运行 cityscapesescript ：
```
